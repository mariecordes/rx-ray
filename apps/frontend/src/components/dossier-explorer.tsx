...
function EvidenceAnswerResult({
  ...
  if (!answer) {
    ...
    return (
      <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-3 text-sm leading-5 text-slate-600">
        {synthesisErrors.length
          ? "The generated evidence summary could not be created, but the retrieved evidence is available below."
          : "Generated evidence summaries are not configured in this environment, but the retrieved evidence is available below."}
      </div>
    );
  }
...
function EvidenceAnswerCard({
  ...
  return (
    <div className="rounded-md border border-[#D7C8F4] bg-[#FBF9FE] p-4">
      <div className="flex items-center gap-2">
        <div className="text-xs font-medium uppercase text-slate-500">
          Evidence summary
        </div>
        <InfoTooltip text="This summary is generated by an LLM from the extracted query state and retrieved public evidence. It is intended for exploration only and does not provide medical advice." />
      </div>
      <p className="mt-2 text-sm leading-6 text-slate-800">{answer.summary}</p>
      ...
      {answer.bullets.length ? (
        <div className="mt-3 space-y-2">
          {answer.bullets.map((bullet, index) => (
            ...
            >
              <p className="text-sm leading-6 text-slate-800">{bullet.text}</p>
              {bullet.citations.length ? (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {bullet.citations.map((citation, citationIndex) => {
                    const sourceNumber = sourceNumberById.get(citation.source_id);
                    return (
                      <Badge
      ...
      {answer.limitations.length ? (
        <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm leading-5 text-amber-900">
      ...
      <p className="mt-3 text-xs leading-5 text-slate-500">{answer.safety_note}</p>
